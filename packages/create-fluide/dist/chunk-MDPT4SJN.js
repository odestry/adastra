...
var banner = async (version) => {
  return console.log(
    `
${label("fluide")}  ${color.hex(brand.colors.yellowgreen).bold(`v${version}`)} ${color.bold(
...
async function typescriptByDefault() {
  await info(
    "Cool!",
    "Astra comes with TypeScript support enabled by default."
  );
  console.log(
    `${" ".repeat(3)}${color.dim(
...
${prefix}Enter your theme directory using`,
  ...
  console.log(
    `${prefix}Add frameworks like ${color.cyan("preact")} or ${color.cyan(
      "tailwind"
    )} by checking ${color.cyan("fluide examples")}.`
  );
  await sleep(100);
  console.log(
    `
${prefix}Stuck? Join us at ${color.cyan(
      "https://support.fluide.build/"
    )}`
  );
  await sleep(200);
}
