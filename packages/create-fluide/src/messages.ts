...
export async function typescriptByDefault(): Promise<void> {
  await info('Cool!', 'Fluide comes with TypeScript support enabled by default.')
  console.log(`${' '.repeat(3)}${color.dim("We'll default to the most relaxed settings for you.")}`)
  await sleep(300)
}

export async function nextSteps({projectDir, devCmd}: {projectDir: string; devCmd: string}): Promise<void> {
