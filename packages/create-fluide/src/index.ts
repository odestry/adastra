...
export async function main (): Promise<void> {
    ...
    await say(
      [
        [
          ...
          'to',
          label('fluide'),
          color.hex(brand.colors.yellowgreen)(`v${version}`) + ',', `${username}!`],
        random(welcome)
      ]
    )
  ...
  // eslint-disable-next-line
  if (!cwd || !isValidProjectDirectory(cwd)) {
    const notEmptyMsg = (dirPath: string): string => `"${bold(dirPath)}" is not empty!`
  ...
  // eslint-disable-next-line
  let tsResponse = args.typescript ||
      (await prompts(
		    {
		      type: 'select',
		      name: 'typescript',
  ...
  if (args.dryRun) {
    ...
    fs.readFile(templateTSConfigPath, (err, data) => {
      ...
      if (templateTSConfig && typeof templateTSConfig === 'object') {
        ...
        console.log(yellow("There was an error applying the requested TypeScript settings. This could be because the template's tsconfig.json is malformed")
        )
      }
    })
