...
import emptyFluideConfig from '../src/utilities/empty-fluide-config'
import fluideConfigPlugin from '../src/plugins/vite-plugin-fluide-config'

describe('fluide-plugin:config', () => {
  afterEach(() => {
    vi.clearAllMocks()
  })

  it('handles a default configuration', () => {
    const options = resolveOptions(emptyFluideConfig)
    const userConfig = fluideConfigPlugin(options)
    // @ts-expect-error
    const config = userConfig.config(emptyFluideConfig, { command: 'serve', mode: 'development' })
    ...
    expect(config.build.manifest).toBe('fluide.json')
  ...
  it('accepts a partial configuration', () => {
    ...
    expect(config.build.rollupOptions.input).toEqual(['src/app.js', 'src/js/app.js'])
  })
})

describe('resolve Fluide Options ', () => {
