...
export const fluideTagDisclaimer =
  '{% comment %}\n  IMPORTANT: This snippet is automatically generated by Fluide.\n  Do not attempt to modify this file directly, as any changes will be overwritten by the next build.\n{% endcomment %}\n'

// Generate liquid variable with resolved path by replacing aliases
export const fluideTagEntryPath = (resolveAlias: Array<{find: string | RegExp; replacement: string}>, entrypointsDir: string): string => {
  ...
  return `{% assign path = fluide | ${replacements.map(([from, to]) => `replace: '${from}/', '${to}/'`).join(' | ')} %}\n`
}

// Generate conditional statement for entry tag
export const fluideEntryTag = (entryPaths: string[], tag: string, isFirstEntry = false): string =>
...
export const stylesheetTag = (fileName: string): string => `{{ '${fileName}' | asset_url | split: '?' | first | stylesheet_tag: preload: preload }}`

// Generate fluide tag snippet for development
export const fluideTagSnippetDev = (assetHost = 'http://localhost:5173', entrypointsDir = 'src', modulesPath = ''): string =>
  `{% liquid
