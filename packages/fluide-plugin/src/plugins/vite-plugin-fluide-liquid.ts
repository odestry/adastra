...
import { disableThemeCheckTag, fluideTagDisclaimer, fluideTagEntryPath, fluideTagSnippetDev, stylesheetTag, scriptTag, preloadScriptTag, fluideEntryTag } from '../utilities'
...
export default (options: ResolvedFluidePluginOptions): Plugin => {
  ...
  const fluideTagSnippetPath = path.resolve(options.themeRoot, `snippets/fluide.liquid`)
  ...
  return {
    ...
    configureServer ({ config }) {
      ...
      const fluideTagSnippetContent = disableThemeCheckTag + fluideTagDisclaimer + fluideTagEntryPath(config.resolve.alias, options.entrypointsDir) + fluideTagSnippetDev(assetHost, options.entrypointsDir, modulesPath)

      // Write fluide tag snippet for development server
      fs.writeFileSync(fluideTagSnippetPath, fluideTagSnippetContent)
    },
    closeBundle () {
      const manifestFilePath = path.resolve(options.themeRoot, `assets/fluide.json`)
      ...
      const fluideTagSnippetContent = disableThemeCheckTag + fluideTagDisclaimer + fluideTagEntryPath(config.resolve.alias, options.entrypointsDir) + assetTags.join('\n') + '\n{% endif %}\n'
