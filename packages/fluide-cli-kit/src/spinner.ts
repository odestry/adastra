 ...
 */
export const gradient = async (text: string): Promise<{
  start: () => void
  stop: () => void
}> => {
  let i = 0
...
export const spinner = async ({ start, end, while: update = async () => await sleep(100) }: { start: string, end: string, while: (...args: any) => Promise<any> }): Promise<void> => {
  const act = update()
