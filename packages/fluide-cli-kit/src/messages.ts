...
export const say = async (messages: string | string[] = [], {clear = false} = {}): Promise<void> => {
  ...
  const eyes = enableAscii() ? ['•', '•', 'o', 'o', '•', 'O', '^', '•'] : ['●', '●', '●', '●', '●', '○', '○', '•']
  const mouths = enableAscii() ? ['•', 'O', '*', 'o', 'o', '•', '-'] : ['•', '○', '■', '▪', '▫', '▬', '▭', '-', '○']
  const walls = enableAscii() ? ['—', '|'] : ['─', '│']
  const corners = enableAscii() ? ['+', '+', '+', '+'] : ['╭', '╮', '╰', '╯']

  const face = (msg: string, {mouth = mouths[0], eye = eyes[0]} = {}): string => {
    const [h, v] = walls
    const [tl, tr, bl, br] = corners
    const head = h.repeat(3)
    return [
      `${tl}${h.repeat(2)}${head}${tr}`,
      `${v} ${eye} ${color.hex(brand.colors.yellowgreen)(mouth)} ${eye} ${color.hex(brand.colors.yellowgreen).bold(brand.tars)}`,
      `${v}${' '.repeat(5)}${v} ${msg}`,
      `${bl}${h.repeat(5)}${br}`,
    ].join('\n')
  }
...
export const label = (text: string, c = color.bgHex(brand.colors.yellowgreen), t = color.white): string => c(` ${t(text)} `)
