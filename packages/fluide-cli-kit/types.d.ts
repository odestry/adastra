...
interface SelectPrompt<Choices extends Readonly<Array<Readonly<Choice>>>> extends BasePrompt {
  type: 'select'
  choices: Choices
}

interface MultiselectPrompt<Choices extends Readonly<Array<Readonly<Choice>>>> extends BasePrompt {
  type: 'multiselect'
  choices: Choices
}
...
type Answer<T extends Prompt, Choices extends Readonly<Choice[]> = T extends SelectPrompt<infer C> ? C : T extends MultiselectPrompt<infer C> ? C : never> = T extends TextPrompt ? string : T extends ConfirmPrompt ? boolean : T extends SelectPrompt<Choices> ? Choices[number]['value'] : T extends MultiselectPrompt<Choices> ? Array<Choices[number]['value'] | undefined> : never
